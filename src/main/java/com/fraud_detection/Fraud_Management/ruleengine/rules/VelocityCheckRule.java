...
public class VelocityCheckRule implements TransactionRule {
    ...
    public TransactionResult apply(TransactionDTO transaction) {
        ...
        if (account == null) {
            return new TransactionResult(TransactionStatus.VALID, "No source account to check velocity");
        }
        ...
        if (lastTime != null) {
            ...
            if (secondsBetween < THRESHOLD_SECONDS) {
                return new TransactionResult(TransactionStatus.ALERT, "Multiple quick successive transactions detected");
            }
        }
